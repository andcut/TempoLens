...
pub fn label_move(
    ...
    let spread = spread_k_cp_mover.unwrap_or(0);
    let complex = punish.max(spread);
    ...
    let time_blunder = dp_practical_mover < cfg.time_blunder_drop && punish < cfg.max_simple_complexity;
    ...
    Label {
        ...
        explanation: format!(
            ...
            spent, complex, dp_practical_mover
        ),
        tips: vec![
            "In blitz, spend time where the position is knife-edge; play instantly where it's not.".to_string(),
        ],
    }
}
